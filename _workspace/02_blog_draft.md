...
- 에이전트별 스킬 정의 (누가 뭘 할 건지 명확히)
- 데이터 흐름 설계 (`_workspace/` 폴더로 에이전트 간 커뮤니케이션)
- 병렬/순서 작업 분류 (FE+BE는 병렬, 통합은 이후)
...
Claude Code의 **Plan mode**를 사용해서:
- 변경할 15개 파일 나열
- **유지할 6개 모듈** 명시적으로 확정 (이쪽은 건드리지 말 것)
- 완료 기준 3가지 정의
...
def get_llm():
    ...
    if backend == "mock":
        ...
        return ClaudeLLM()  # 나중에 추가
    elif backend == "huggingface":
        return HuggingFaceLLM()  # 나중에 추가
...
근데 이건 느낌이 이상했다. 게임은 "김지윤을 **키우는** 게임"인데, 플레이어가 여전히 **제3자**처럼 느껴졌다.
...
### LLM 모델 연동 없는 상태
현재 백엔드는 Mock LLM으로 하드코딩된 응답만 한다. 실제로 플레이어 입력에 따라 NPC가 다르게 응답하려면 HuggingFace API 또는 Claude API를 연동해야 한다. 다음 단계(Day 4)에서 `llm-integrator` 에이전트가 이를 처리할 예정이다.
